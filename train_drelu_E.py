...
def train(config):
    ...
    dehaze_net = model_depth_Drelu.MSDFN_CUT_E().cuda()
    #dehaze_net.apply(weights_init)
    dehaze_net.load_state_dict(torch.load("/home/amax/share/FGD/MSDFN/trained_model/indoor/CUT_E_drelu-B/Epoch8.pth"))
    ...
    indexX = []                                                                                                    #计数损失曲线用
    indexY = []
    for epoch in range(8,config.num_epochs):
        if epoch == 0:
            config.lr = 0.0001
        elif epoch == 1:
            config.lr = 0.00009
        ...
        # plt.pause(0.1)
        # plt.plot(indexY, indexX)

        #plt.savefig("IRDN/%iepoch.png" % epoch)                                                                        #early——stop机制
        # if zt == 0 and Iters >= 700:
        #     break
        ...
        with torch.no_grad():
            for iteration, (img_clean, img_haze,img_depth) in enumerate(val_loader):
                ...
                torchvision.utils.save_image(clean_image,config.sample_output_folder + "/epoch%s" % epoch + "/" + str(iteration + 1) + ".jpg")
            ...
            with open("trainlog/indoor/CUT_E_%s%s.log" % (config.lossfunc,config.actfuntion), "a+", encoding="utf-8") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--snapshots_folder', type=str, default="trained_model/indoor/CUT_E_drelu-B/")
    parser.add_argument('--sample_output_folder', type=str, default="sample/indoor/CUT_E_drelu-B")
    ...
    if config.in_or_out == 'outdoor':
        ...
        config.snapshots_folder = "trained_model/outdoor/CUT_E_drelu-B/"
        config.sample_output_folder = "sample/outdoor/CUT_E_drelu-B"
